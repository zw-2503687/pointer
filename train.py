...
class Trainer(object):
    ...
    def __call__(self):
        ...
        batch_size_val = 1
        ite_num = 0
        data_loader_train = DataLoader(self.data_set_train, batch_size_train, True, num_workers=4)
        ...
        for epoch in range(epoch_num):
            ...
            for i, (images, masks) in enumerate(data_loader_train):
                ite_num += 1
                images = images.to(self.device)#[3,3,416,416]
                masks = masks.to(self.device)#[3,2,416,416] 将刻度和指针分割成2个通道
                #前向传播
                d0, d1, d2, d3, d4, d5, d6 = self.net(images)#不同尺度的图像[3,2,416,416]
                #计算损失
                ...
                #更新参数
                self.optimizer.step()
                loss_sum += loss.item()#其实就是loss
                running_tar_loss += loss0.item()#记录预测与真实的损失
